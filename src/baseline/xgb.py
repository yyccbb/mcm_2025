...
# 4. 训练模型
# 模型1：预测金牌数
model_gold = XGBRegressor(objective='reg:squarederror', n_estimators=100, max_depth=3, learning_rate=0.1, random_state=42)
model_gold.fit(X_train_scaled, y_gold_train)


# 模型2：预测总奖牌数
model_total = XGBRegressor(objective='reg:squarederror', n_estimators=100, max_depth=3, learning_rate=0.1, random_state=42)
model_total.fit(X_train_scaled, y_total_train)


# 5. 模型预测
gold_pred = model_gold.predict(X_test_scaled)
total_pred = model_total.predict(X_test_scaled)
